    ...
    <div class="ticket-stage">
            ...
            <div class="ticket-paper">
                ...
                <div class="ticket-content">
                    ...
                    <div class="ticket-headline">
                        ...
                        <p class="ticket-vendor">Servi par {{ $vendeur_info['prenom'] ?? '' }} {{ $vendeur_info['nom'] ?? '' }}</p>
                        @endif
                    </div>
                    ...
                    <div class="ticket-body">
                        <div class="ticket-row">
                            ...
                            <span class="ticket-value ticket-pw">
                                <span id="tkUser">{{ $ticket->user }}</span>
                                <button type="button" class="copy-btn" data-copy="{{ $ticket->user }}" aria-label="Copier l'utilisateur">
                        ...
                        <div class="ticket-row">
                            ...
                            <span class="ticket-value ticket-pw">
                                <span class="pw-text" data-pw="{{ $ticket->password }}" data-revealed="0" id="tkPass">••••••</span>
                                <button type="button" class="pw-toggle" id="pwToggle" aria-label="Afficher le mot de passe">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button type="button" class="copy-btn" data-copy="{{ $ticket->password }}" aria-label="Copier le mot de passe">
                                    <i class="fas fa-copy"></i>
                                </button>
                            </span>
                        </div>
                        ...
                        <div class="ticket-row ticket-row-total">
                            ...
                            <span class="ticket-value ticket-amount">
                                {{ number_format($ticket->montant ?? 0) }} <small>{{ config('platform.currency', 'XOF') }}</small>
                            </span>
                        </div>
                        <div class="ticket-row">
                            <span class="ticket-label">Token</span>
                            <span class="ticket-value ticket-pw">
                                <span id="tkToken">{{ $token }}</span>
                                <button type="button" class="copy-btn" data-copy="{{ $token }}" aria-label="Copier le token">
                                    <i class="fas fa-copy"></i>
                                </button>
                            </span>
                        </div>
                    ...
                    <div class="ticket-actions">
                        ...
                        <button type="button" class="ticket-btn ticket-btn-primary" id="btnConnect"
                                data-user="{{ $ticket->user }}" data-password="{{ $ticket->password }}">
                    ...
                    <div class="ticket-stub">
                        ...
                        <div class="ticket-stub-id">N° {{ $ticketRef }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            (function () {
                ...
                function downloadPDF() {
                    ...
                    var user = {!! json_encode($ticket->user ?? '') !!};
                    var forfait = {!! json_encode($ticket->forfait ?? '') !!};
                    var montant = {!! json_encode(number_format($ticket->montant ?? 0) . ' ' . config('platform.currency', 'XOF')) !!};
                    var platformName = {!! json_encode(config('platform.name')) !!};
                    ...
                    doc.setFontSize(10);
                    doc.setTextColor(140, 133, 112);
                    doc.text("Assistance : {{ $supportCallNumber }} / {{ $supportWhatsapp }}", 105, y + 16, { align: 'center' });
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    if (connectBtn) {
                        connectBtn.addEventListener('click', function () {
                            ...
                            window.location.href = "{{ $mikrotikUrl }}/login.html?username=" + encodeURIComponent(user) + "&password=" + encodeURIComponent(password);
                        });
                    }
                    ...
                    if (qrContainer && window.QRCode) {
                        var user = {!! json_encode($ticket->user ?? '') !!};
                        var password = {!! json_encode($ticket->password ?? '') !!};
                        var loginUrl = "{{ $mikrotikUrl }}/login.html?username=" + encodeURIComponent(user) + "&password=" + encodeURIComponent(password);
        ...
        <div class="ticket-3d" id="ticketCard">
            ...
            <div class="ticket-paper ticket-paper-pending">
                ...
                <div class="ticket-content">
                    ...
                    <div class="ticket-headline">
                        <h2>{{ ($token ?? null) ? 'Paiement reçu' : 'Ticket introuvable' }}</h2>
                    </div>
                    ...
                    <div class="ticket-body-pending">
                        @if($token ?? null)
                            ...
                            <div class="ticket-token">Token : {{ $token }}</div>
                        @else
                            <p>Aucune transaction trouvée pour ce paiement.</p>
                        @endif
                    </div>

                    <a href="{{ url('/') }}" class="ticket-btn ticket-btn-secondary ticket-btn-link">
        ...
        <div class="support-card">
            ...
            <div class="support-channels">
                <a href="tel:{{ $supportCallTel }}" class="support-channel">
                    <span class="support-icon"><i class="fas fa-phone-alt"></i></span>
                    <span class="support-text">
                        <strong>Appelez-nous</strong>
                        <span>{{ $supportCallNumber }}</span>
                    </span>
                    <i class="fas fa-chevron-right support-arrow"></i>
                </a>
                <a href="https://wa.me/{{ $supportWaLink }}" target="_blank" rel="noopener noreferrer" class="support-channel whatsapp">

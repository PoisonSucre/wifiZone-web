...
<body style="margin:0;padding:0;background-color:#eef1f5;font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
    ...
    <div style="display:none;max-height:0;overflow:hidden;mso-hide:all;opacity:0;">
        Un paiement de {{ number_format($transaction->montant, 0, ',', ' ') }} {{ config('platform.currency') }} a été reçu avec succès sur {{ config('platform.name') }}.
    </div>
                    ...
                    <tr>
                        <td class="email-padding" style="padding:44px 44px 36px;">
                            ...
                            <p style="margin:0 0 22px;color:#475569;font-size:15px;line-height:1.7;text-align:center;">
                                ...
                                un paiement de <strong style="color:#059669;">{{ number_format($transaction->montant, 0, ',', ' ') }} {{ config('platform.currency') }}</strong> a été reçu avec succès.
                            </p>
                                ...
                                <tr>
                                    <td style="padding:18px 22px;">
                                        ...
                                        <p style="margin:0;color:#065f46;font-size:13px;line-height:1.8;">
                                            Utilisateur : <strong>{{ $ticket->user }}</strong><br>
                                            Forfait : <strong>{{ $ticket->forfait }}</strong><br>
                                            Montant : <strong>{{ number_format($ticket->montant, 0, ',', ' ') }} {{ config('platform.currency') }}</strong>
                                        </p>
                                    </td>
                                </tr>
                            </table>
                            ...
                            <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto 20px;">
                                <tr>
                                    <td align="center" style="border-radius:10px;background-color:#059669;">
                                        <a href="{{ url('/vendeur') }}" class="cta-button" style="display:inline-block;padding:15px 44px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;border-radius:10px;">

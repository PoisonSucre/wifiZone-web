...
<div x-data="{ loaded: false, cardFlipped: false, retraitOpen: false, commentCaMarcheOpen: false }" x-cloak
    ...
    <div x-show="loaded" x-cloak
        ...
        {{-- ============================================================ --}}
        {{-- CARTE VENDEUR (hero) — front: solde / back: détail des gains --}}
        {{-- ============================================================ --}}
                ...
                <div class="group absolute inset-0 rounded-3xl overflow-hidden [backface-visibility:hidden]
                    ...
                    <div class="relative h-full flex flex-col justify-between p-5">
                        ...
                        <div>
                            ...
                            <p class="mt-2 font-mono text-xs sm:text-sm text-white/35 tracking-[0.25em]">{{ $cardNumber }}</p>
                        </div>
                    ...
                    <div class="relative p-5 flex flex-col h-[calc(100%-2.25rem)]">
                        ...
                        <div class="grid grid-cols-3 gap-2 flex-1">
                            <div class="flex flex-col justify-center">
                                ...
                                <p class="font-mono font-black text-white text-xs sm:text-sm tabular-nums">{{ number_format($totalRevenus, 0, ',', ' ') }}</p>
                            </div>
                            <div class="flex flex-col justify-center">
                                <p class="text-[8px] font-bold text-white/35 uppercase tracking-wider mb-1">Retiré</p>
                                <p class="font-mono font-black text-orange-400 text-xs sm:text-sm tabular-nums">{{ number_format($dejaRetire, 0, ',', ' ') }}</p>
                            </div>
                            <div class="flex flex-col justify-center">
                                <p class="text-[8px] font-bold text-white/35 uppercase tracking-wider mb-1">Net ({{ $commissionPct }}%)</p>
                                <p class="font-mono font-black text-neonGreen text-xs sm:text-sm tabular-nums">{{ number_format($soldeNet, 0, ',', ' ') }}</p>
                            </div>
                        </div>
                        ...
                        @forelse($retraits as $retrait)
                            <tr class="border-b border-slate-50 dark:border-darkBorder/20 hover:bg-slate-50/70 dark:hover:bg-darkBg/40 transition-colors last:border-0 group">
                                <td class="py-2.5 px-4 text-slate-500 dark:text-gray-400 whitespace-nowrap">
                                    {{ $retrait->date_creation ? $retrait->date_creation->format('d/m/Y H:i') : '-' }}
                                </td>
                                ...
                                <td class="py-2.5 px-4 font-black text-neonGreen dark:text-neonGreen whitespace-nowrap">
                                    {{ number_format($retrait->montant_net, 0, ',', ' ') }} <span class="text-[9px] text-slate-400">{{ $currency }}</span>
                                </td>
                                <td class="py-2.5 px-4 text-right">
                                    ...
                                    <div class="flex flex-col items-end gap-0.5">
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold {{ $statusStyle }}">
                    ...
                    <form wire:submit.prevent="demanderRetrait" class="space-y-4">
                        <div>
                            ...
                            <div class="flex items-center gap-1.5 mt-2">
                                @foreach(['25%' => $pct25, '50%' => $pct50, 'Max' => $pctMax] as $label => $value)
                                    <button type="button"
                                            @click="$refs.montantInput.value = {{ $value }}; $refs.montantInput.dispatchEvent(new Event('input'))"
                            ...
                            @error('montant') <p class="text-red-500 text-[11px] mt-1 font-bold">{{ $message }}</p> @enderror
                        </div>
                        <div>
            ...
            <div class="space-y-4">
                @foreach([
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <p class="text-xs text-slate-600 dark:text-gray-400">{{ $step['text'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>

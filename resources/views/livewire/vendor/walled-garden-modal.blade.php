    ...
    <div class="bg-white dark:bg-darkCard border border-slate-200/80 dark:border-darkBorder rounded-2xl shadow-2xl overflow-hidden max-w-lg w-full max-h-[92vh] flex flex-col"
        ...
        <div class="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100 dark:border-darkBorder/40 bg-slate-50/50 dark:bg-darkBg/30 shrink-0">
            ...
            <div class="flex-1">
                ...
                <p class="text-[10px] text-slate-400 dark:text-gray-500">Hotspot : {{ $hotspotName }}</p>
            </div>
        ...
        <div class="p-5 overflow-y-auto flex-1 min-h-0 space-y-3">
            ...
            <div class="relative">
                ...
                <pre id="walledGardenConfig" class="bg-slate-900 dark:bg-black/60 border border-slate-700 dark:border-darkBorder rounded-xl p-4 overflow-x-auto text-[10px] leading-relaxed text-slate-300 dark:text-gray-300 font-mono max-h-[350px] overflow-y-auto whitespace-pre">{{ $config }}</pre>
            </div>

            {{-- Info URL MikroTik --}}
            @if(empty($mikrotikUrl))
